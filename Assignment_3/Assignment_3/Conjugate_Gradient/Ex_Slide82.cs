...
namespace Conjugate_Gradient
{
    class Ex_Slide82
    {
        ...
        {
            return (x2 - Math.Pow(x1, 2)) * (x2 - Math.Pow(x1, 2)) + (x2 - 2 * x1 + 1) * (x2 - 2 * x1 + 1) + (x1 + x2 - 2) * (x1 + x2 - 2);
        }
        //partial derivative after x1
        static double X1der(double x1, double x2)
        {
            return (4 * x1 * x1 * x1) +( x1 * (10 - (4 * x2))) - (2 * (x2 + 4));
        }
        //partial derivative after x2
        static double X2der(double x1, double x2)
        {
            return -2 * ((x1 * x1) + x1 - (3 * x2) + 1);
        }
        public static double[] initialValue = { -3, -3 };
        ...
        public static string Conjugate_Gradient(double Eps, double[] initialValue)
        {
            ...
            do
            {
                ...
                Pk[0] = -Gradient(initialValue[0], initialValue[1])[0] + (Pk[0] * Bk);
                Pk[1] = -Gradient(initialValue[0], initialValue[1])[1] + (Pk[1] * Bk);
